...
import { useNavigate } from "react-router-dom";export default function Cart(){
    ...
    <Typography variant="h6" sx={{fontWeight:'bold'}}>
     ...
     <Grid md={2}>
        <Typography variant="h6" sx={{fontWeight:'bold'}}> ActualPrice</Typography>
         { cartList.map((item)=>{ 
            return( <Typography> {item.actualPrice}
        </Typography> )
         })
         } 
         </Grid>
         <Grid md={2}>
        <Typography variant="h6" sx={{fontWeight:'bold'}}> DiscountPrice</Typography>
         { cartList.map((item)=>{ 
            return( <Typography> {item.discountPrice}
        </Typography> )
         })
         } 
         </Grid>
        ...
        <Typography variant="h6" sx={{fontWeight:'bold'}}>Amount</Typography>
         { cartList.map((item)=>{ 
            ...
            return( <Typography> {item.discountPrice*item.qty}
        </Typography> )
         })
         } 
